...
%%
% Set the linespacing from setspace package:
\setstretch{1.1}
...
\begin{figure*}[p]
\fbox{\includegraphics[width=0.45\linewidth]{graphics/vdqi-title}}
\hfill
\fbox{\includegraphics[width=0.45\linewidth]{graphics/ei-title}}
\\\vspace{\baselineskip}
\fbox{\includegraphics[width=0.45\linewidth]{graphics/ve-title}}
\hfill
\fbox{\includegraphics[width=0.45\linewidth]{graphics/be-title}}
\end{figure*}
...
\fbox{\includegraphics[width=0.45\linewidth]{graphics/vdqi-contents}}
\hfill
\fbox{\includegraphics[width=0.45\linewidth]{graphics/ei-contents}}
\\\vspace{\baselineskip}
\fbox{\includegraphics[width=0.45\linewidth]{graphics/ve-contents}}
\hfill
\fbox{\includegraphics[width=0.45\linewidth]{graphics/be-contents}}
\end{figure*}


\section{Typefaces (Modified)}\label{sec:typefaces1}\index{typefaces}
...
installations, 
% the \TL document classes default to using Palatino and Helvetica, respectively.  In addition, the Bera Mono typeface is used for \texttt{monospaced} type.
these modified \TL document classes default to using ETbb. More specifically the fonts are loaded as follows:
\begin{docspec}
\textbackslash usepackage[p]\{ETbb\}\\
\textbackslash usepackage[scaled=0.90,type1]\{cabin\}\\
\textbackslash usepackage[varqu,varl]\{zi4\}\\
\textbackslash usepackage[libertine,vvarbb]\{newtxmath\}
\end{docspec}
The fonts will look like this for \textsf{sans serif} and like this for \texttt{typewriter}.

%The following font sizes are defined by the \TL classes:
The font sizes have been made dependent on the base class font sizes, i.e. article for tufte-handout and book for tufte-book. %
\begingroup%
\makeatletter%
\newcommand{\myfontsize}{\f@size}%
\makeatother%
This line here was typeset at a font size of~\myfontsize{}pt.
\endgroup
The line spacing of the \TL documents is controlled by the setspace package, which is loaded automatically; for this document we used in the preamble:
\begin{docspec}
\textbackslash setstretch\{1.1\}
\end{docspec}
...
chapters\sidenote{Parts and chapters are defined for the \texttt{tufte-book}% end texttt
class only.}, % end sidenote.
sections, subsections, and paragraphs.  Not defined by default
are: sub-subsections and subparagraphs, but subsubsection has been enabled here.
...
\begin{table}[h]
  \begin{center}
    ...
    \begin{tabular}{lcr}
      ...
      Subsubsection & italic & \measure{11}{15}{26} \\
...
\section{Analysis and Tests}



\subsubsection{Mathematical Text}

\textbf{Simplest form of the \textit{Central Limit Theorem}\footnote{quoted from the ETbb package documentation, where it is used as illustration in the same way.}:} \textit{Let
...
\begin{figure*}[h]
  \includegraphics[width=\linewidth]{sine}%
...
The font size can be controlled with the options \docclsoptdef{10pt}, \docclsoptdef{11pt} or \docclsoptdef{12pt}. The default if no option is given will be \docclsoptdef{10pt}. The font adjustments to \docclsoptdef{10pt} done by the original \TL classes have been removed, they now lie with the font size definitions of the base class.
